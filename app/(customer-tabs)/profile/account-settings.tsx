...
import { ArrowRight, ArrowLeft, CircleUserRound, LockKeyhole, X, KeyRound, Trash2, Camera } from 'lucide-react-native';
...
export default function CustomerAccountSettingsScreen() {
  const { colors, t, user, updateUser, isRTL, logout, updateAuthPassword, isFirebaseConfigured } = useApp();
  ...
  const handleSave = async () => {
    await updateUser({ name, email, phone, avatar: avatarUri || undefined });
  ...
  const handleChangePassword = async () => {
    ...
    if (!isFirebaseConfigured()) {
      setAlertConfig({ visible: true, type: 'error', message: 'تغيير كلمة المرور يتطلب الاتصال بالخادم.' });
      return;
    }
    setIsChangingPassword(true);
    const updateResult = await updateAuthPassword(newPassword);
    setIsChangingPassword(false);
    if (!updateResult.success) {
      setAlertConfig({ visible: true, type: 'error', message: updateResult.error || 'فشل تغيير كلمة المرور' });
      return;
    }
        ...
        <TouchableOpacity style={styles.avatarSection} onPress={handlePickAvatar} activeOpacity={0.7}>
          {avatarUri ? (
            ...
            <View style={[styles.avatarPlaceholder, { backgroundColor: colors.primaryLight }]}>
              <CircleUserRound size={40} color={colors.primary} />
            </View>
          )}
      ...
      <Modal visible={showPasswordModal} transparent animationType="slide">
        <View style={styles.modalOverlay}>
          <View style={[styles.modalContent, { backgroundColor: colors.glass, borderColor: colors.glassBorder, borderWidth: 1 }]}>
            ...
            <TouchableOpacity
              style={[styles.modalBtn, { backgroundColor: colors.primary, opacity: newPassword && newPassword === confirmPassword && !isChangingPassword ? 1 : 0.5 }]}
              onPress={handleChangePassword}
              disabled={!newPassword || newPassword !== confirmPassword || isChangingPassword}
            >
              <Text style={styles.modalBtnText}>{isChangingPassword ? (t('saving') || 'جاري الحفظ...') : t('confirm')}</Text>
            </TouchableOpacity>
          </View>
        </View>
      </Modal>
