...
import { Search, X, Trash2, CircleCheck, Circle, CircleUserRound } from 'lucide-react-native';
...
function merchantPeerDisplayName(c: DirectConversation): string {
  ...
  if (/^customer_\d+$/i.test(id)) return `#${id.slice('customer_'.length)}`;
  return id || '—';
}

function directToConversationMerchant(c: DirectConversation): Conversation {
  ...
  return {
    id: c.id,
    name: merchantPeerDisplayName(c),
    avatar: c.customerImage || '',
    lastMessage: c.lastMessage || '',
    time,
    unreadCount: c.unreadCountMerchant ?? 0,
    isOnline: false,
  };
}

export default function MerchantMessagesScreen() {
  const { colors, t, isRTL, user } = useApp();
  ...
  useEffect(() => {
    ...
    const unsub = subscribeToMerchantDirectConversations(merchantId, (list) => {
      setConversations(list.map(directToConversationMerchant));
      setLoading(false);
    });
    return unsub;
  }, [merchantId]);
    ...
    >
      ...
      <View style={styles.avatarContainer}>
        {item.avatar ? (
          ...
          <View style={[styles.avatarPlaceholder, { backgroundColor: colors.borderLight }]}>
            <CircleUserRound size={28} color={colors.textMuted} />
          </View>
        )}
      ...
      <View style={styles.convLeft}>
        ...
        <Text style={[styles.convTime, {
          color: item.unreadCount > 0 ? colors.primary : colors.textMuted,
          fontWeight: item.unreadCount > 0 ? '700' as const : '400' as const,
        }]}>{item.time}</Text>
      </View>
    </TouchableOpacity>
  );
...
const styles = StyleSheet.create({
  ...
  convLeft: { alignItems: 'flex-start', gap: 4 },
  unreadBadge: {
    width: 22,
    height: 22,
    borderRadius: 11,
