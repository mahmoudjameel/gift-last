...
export default function EditProductScreen() {
  const { colors, t, language, isRTL, products, updateProduct, deleteProduct, categories: appCategories, user } = useApp();
  ...
  const [productNumber, setProductNumber] = useState(product?.sku || product?.id || '');
  const [name, setName] = useState(product?.name ?? '');
  const [desc, setDesc] = useState(product?.description ?? '');
  const hasExistingDiscount = product?.originalPrice != null && product.originalPrice > product.price;
  const [price, setPrice] = useState(hasExistingDiscount ? (product.originalPrice ?? 0).toString() : (product?.price?.toString() ?? ''));
  ...
  const [discountPrice, setDiscountPrice] = useState(product?.originalPrice ? product.price.toString() : '');
  const [hasGift, setHasGift] = useState(product?.hasGiftCard || false);
  const [giftFee, setGiftFee] = useState(product?.giftCardFee?.toString() || '');
  const [options, setOptions] = useState<ProductOption[]>(() =>
    (product?.options || []).filter((o) => o.type === 'multiple_choice')
  );
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      ...
      setAlertConfig({ visible: true, type: 'error', message: 'حدث خطأ أثناء حفظ التعديلات' });
    } finally {
      setIsSaving(false);
    }
  }, [product, name, desc, price, qty, category, images, mainImageIndex, isHidden, hasGift, giftFee, hasDiscount, discountPrice, options, productNumber, updateProduct, t, router]);
  ...
  const handleDelete = useCallback(async () => {
    ...
    try {
      ...
      setAlertConfig({ visible: true, type: 'error', message: 'حدث خطأ أثناء حذف المنتج' });
      setShowDeleteModal(false);
    } finally {
      setIsDeleting(false);
    }
  ...
  return (
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      ...
      >
        <SafeAreaView edges={['top']}>
          <View style={styles.header}>
            ...
            <Text style={styles.headerTitle}>{t('editProductTitle')}</Text>
            <View style={{ width: 36 }} />
          </View>
          <Text style={styles.headerDesc}>{t('editProductDesc')}</Text>
        </SafeAreaView>
      </LinearGradient>
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scrollContent}>
        ...
        {isHidden && (
          <View style={[styles.hiddenBanner, { backgroundColor: '#F59E0B18' }]}>
            <EyeOff size={16} color="#F59E0B" />
            <Text style={styles.hiddenBannerText}>
              {language === 'ar' ? 'هذا المنتج مخفي حالياً ولا يظهر للعملاء' : 'This product is currently hidden'}
            </Text>
          </View>
        )}
        ...
        >
          ...
          >
            <Text style={styles.saveButtonText}>{isSaving ? 'جاري الحفظ...' : t('saveChanges')}</Text>
          </LinearGradient>
        </TouchableOpacity>

        <View style={styles.actionButtons}>
          <TouchableOpacity
            style={[styles.actionBtn, { backgroundColor: colors.card, borderColor: colors.borderLight, borderWidth: 1 }]}
          ...
          <TouchableOpacity
            style={[styles.actionBtn, { backgroundColor: '#EF444410', borderColor: '#EF444425', borderWidth: 1 }]}
            onPress={() => setShowDeleteModal(true)}
            activeOpacity={0.7}
          >
            <Trash2 size={18} color="#EF4444" />
            <Text style={[styles.actionBtnText, { color: '#EF4444' }]}>{t('deleteProduct')}</Text>
          </TouchableOpacity>
        </View>
      ...
      <Modal visible={showDeleteModal} transparent animationType="fade">
        <View style={styles.deleteOverlay}>
          <View style={[styles.deleteModal, { backgroundColor: colors.card }]}>
            ...
            <Text style={[styles.deleteModalTitle, { color: colors.text }]}>{t('deleteProduct')}</Text>
            <Text style={[styles.deleteModalDesc, { color: colors.textSecondary }]}>
              {language === 'ar' ? 'هل أنت متأكد من حذف هذا المنتج؟ لا يمكن التراجع عن هذا الإجراء.' : 'Are you sure you want to delete this product? This cannot be undone.'}
            </Text>
            <View style={styles.deleteModalBtns}>
              ...
              <TouchableOpacity
                style={[styles.deleteModalBtn, { backgroundColor: '#EF4444', opacity: isDeleting ? 0.6 : 1 }]}
                onPress={handleDelete}
                disabled={isDeleting}
              >
                <Text style={[styles.deleteModalBtnText, { color: '#FFF' }]}>{isDeleting ? 'جاري الحذف...' : t('delete')}</Text>
              </TouchableOpacity>
            </View>
          </View>
        </View>
